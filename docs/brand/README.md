...
`app-icon/logo-1000x300.svg` is the wide add-on logo — the tile and the
wordmark, on a transparent ground. It is generated by `build-wordmark.py`
rather than drawn, so it follows the wordmark automatically.

Home Assistant wants PNG for both. See below.
...
2. `addon/maverick-wall/logo.png` — 1000x300, built from
   `app-icon/logo-1000x300.svg`. **Transparent background**: one PNG has to sit
   on both of Home Assistant's themes, and a plate that reads as a deliberate
   badge on a white card reads as a stray dark box on a dark one. Only the mark
   carries a ground — it is the app icon, and the mark needs one, because a
   quiet field with one cell lit cannot be quiet with nothing behind it. The
   wordmark stands on whatever is there, so its `#A8701A` is chosen for the
   worse of the two grounds: 4.20:1 on white, 4.06:1 on Home Assistant's dark
   card. That value is derived for this constraint, not lifted from
   `theme.ts` — Board's `#E8A33D` is 2.16:1 on white and Almanac's `#C98A16` is
   2.95:1, and both wash out on a light card
