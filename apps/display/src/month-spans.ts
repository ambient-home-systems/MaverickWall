/**
 * A multi-day event is drawn *once*, as a bar across the days it covers.
 *
 * The month grid used to build one row per event per cell, so "Half term" over
 * seven days spent a row in seven consecutive squares and printed the same two
 * words seven times — at exactly the moment a row is the scarcest unit on the
 * wall. Measured on the shipped Classic wall in portrait, a 114x129 cell has
 * room for **one** row, so a week of half term was the whole month grid saying
 * one thing.
 *
 * `continues` has been on every event in the manifest since the server started
 * bucketing an event onto each date it touches (`buildManifest`: "`continues`
 * lets the display draw it as a bar rather than repeating the title"). Nothing
 * read it. This is the reading.
 *
 * Four rules, and each of them is a trap this got wrong first:
 *
 * - **A run stops at the week boundary.** The grid is seven columns; a bar
 *   cannot wrap. So a fortnight is two bars, and only the first one carries
 *   the title — a title repeated on the second row is the bug being fixed, one
 *   row down. `leading` is a fact about the *whole grid*, not about the week,
 *   which is why `monthSpans` walks the weeks in order and remembers.
 * - **Length comes from the occurrences, never from the dates.** `DTEND` is
 *   exclusive — this repository's own notes call it the single most common ICS
 *   bug — so subtracting one civil date from another puts every birthday on
 *   the wrong day and every half term a day long. The server has already
 *   bucketed the event onto each date it touches; counting those buckets is
 *   the arithmetic-free answer and cannot be off by one.
 * - **Only all-day events span.** A timed event that recurs daily is seven
 *   separate occurrences of one series and stays one row per day; a *single*
 *   overnight event is `continues` too, and drawing it as a bar would claim a
 *   whole second day for something that ended at one in the morning. The test
 *   is `allDay && continues`, and both halves are load-bearing.
 * - **Two events over the same days stack.** A bar takes the lowest lane that
 *   is free across every column it covers, so overlapping spans sit one above
 *   the other rather than one on top of the other.
 *
 * Pure, and in its own file, for the reason `widget-options.ts`, `ink.ts` and
 * `ladder.ts` are: there is no DOM in this package's tests, so a rule resolved
 * inside a `createElement` call is a rule nothing can check. It is transcribed
 * to `apps/server/src/epaper/month-spans.ts` — the display bundle has no
 * bundler and the server cannot import it — and `month-spans-parity.test.ts`
 * reads both files and compares what each one says. **The wall is the spec.**
 */
