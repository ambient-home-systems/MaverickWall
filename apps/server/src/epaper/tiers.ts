/**
 * Density tiers, as the panel reads them — `apps/display/src/tiers.ts`
 ...
 * it without falling outside `tsconfig.test.json`'s root. `ladder.ts`,
 * `calendar-view.ts` and `month-spans.ts` all have the same seam for the same
 * reason, and the same guard: `tier-parity.test.ts` reads *both* files and
 * holds everything below this comment to being character-identical, in both
 * directions and as *sets* — a table added on one side alone is exactly what
 * the ladder parity test missed in its first version.
 *
 * A panel following a wall has to draw the same month, and "the same month" now
 * includes what form each cell takes. Resolving that from a second opinion is
 * the fault this repository has shipped under four names — `shifts[0]`,
 * `display_mode`, `cellEvents`, `mode` — and the cure each time was to resolve
 * it once and hand over the answer. **The wall is the spec**; this is a copy of
 * the wall's reading and must never become an opinion about it.
 *
